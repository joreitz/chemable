...
export function generateXYZ(atoms: Atom[]): string {
    ...
    xyz += "Generated by Chemable Editor\n";
    
    atoms.forEach(atom => {
        const x = (atom.x * 0.05).toFixed(4);
        const y = (atom.y * -0.05).toFixed(4);
        const z = "0.0000";
...
export function convertSdfToXyz(sdfData: string): string {
    ...
    let xyz = `${numAtoms}\n`;
    xyz += "Generated by Chemable Editor (3D)\n";
    ...
    for (let i = 0; i < numAtoms; i++) {
        ...
        if (parts.length >= 4) {
            const x = parseFloat(parts[0]).toFixed(4);
            const y = parseFloat(parts[1]).toFixed(4);
            const z = parseFloat(parts[2]).toFixed(4);
